...
def run_sim(x, y, metamodel, output_dir, model_template, counter=[0], history=[]):
    ...
    new_office_description = model_template.render(
...
for index, row in df.iterrows():
    ...
    problem = Problem(nvars=2, nobjs=3, function=f_partial)
    problem = Problem(nvars=2, nobjs=3, function=f_partial)
    # problem.types[0] = Subset(['a', 'b'], 1)
    problem.types[0] = Real(1, 20)
    problem.types[1] = Subset(range(1, 13), 1)
    # problem.types[1] = Integer(1, 13)
    algorithm = NSGAII(problem, population_size=5, variator=SSX())
    algorithm.run(20)
